...
if [ -z "${MOUNT_CHECK}" ] ; then
  ...
  echo -e "network tools busybox - ${HOSTNAME} - ${CONTAINER_IP} - HTTP: ${HTTP_PORT:-80} , HTTPS: ${HTTPS_PORT:-443} . " | tee  ${WEB_ROOT}/index.html 
else
  ...
  echo -e "network tools busybox - ${HOSTNAME} - ${CONTAINER_IP} - HTTP: ${HTTP_PORT:-80} , HTTPS: ${HTTPS_PORT:-443} . "
